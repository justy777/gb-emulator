...
const MEM_BACKGROUND_Y: u16 = 0xFF42;
const MEM_BACKGROUND_X: u16 = 0xFF43;
const MEM_SCANLINE: u16 = 0xFF44;
const MEM_SCANLINE_COMPARE: u16 = 0xFF45;
...
impl Ppu {
    ...
    pub const fn read_display(&self, address: u16) -> u8 {
        match address {
            MEM_DISPLAY_CONTROL => self.display_control.bits(),
            MEM_DISPLAY_STATUS => self.display_status.bits(),
            MEM_BACKGROUND_Y => self.background_y,
            MEM_BACKGROUND_X => self.background_x,
            MEM_SCANLINE => self.scanline,
            MEM_SCANLINE_COMPARE => self.scanline_compare,
    ...
    pub fn write_display(&mut self, address: u16, value: u8) {
        match address {
            ...
            MEM_DISPLAY_STATUS => self.display_status = DisplayStatus::from_bits_truncate(value),
            MEM_BACKGROUND_Y => self.background_y = value,
            MEM_BACKGROUND_X => self.background_x = value,
            // LY is read-only
            MEM_SCANLINE_COMPARE => {
