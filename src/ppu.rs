...
impl Ppu {
    ...
    pub fn step(&mut self, cycles: usize, interrupt_flag: &mut InterruptFlags) {
        ...
        while cycle_count > 0 {
            if ppu_mode == PpuMode::HBlank {
                ...
                if dot == 0 {
                    ...
                    self.display_status.set(DisplayStatus::COINCIDENCE, value);
                }
                ...
                if self.sprite_buffer.len() < 10
                    ...
                    && self.scanline_y + 16 <= sprite.y + sprite_height
                {
    ...
    pub fn write_display(&mut self, address: u16, value: u8) {
        match address {
            MEM_DISPLAY_CONTROL => self.display_control = DisplayControl::from_bits_truncate(value),
