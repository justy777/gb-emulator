...
impl BusInterface for AddressBus<'_> {
    fn read(&self, addr: u16) -> u8 {
        match addr {
            ...
            0xFE00..=0xFEFF => self.ppu.read_oam(addr - 0xFE00),
    ...
    fn write(&mut self, addr: u16, value: u8) {
        match addr {
            ...
            0xFE00..=0xFEFF => self.ppu.write_oam(addr - 0xFE00, value),
    ...
    fn tick(&mut self) {
        let apu_event = self.timer.increment_divider(self.interrupt_flags);
        self.apu.tick_clock();
        if apu_event {
            self.apu.tick_divider();
        }
