...
impl Cpu {
    ...
    fn execute_prefixed(&mut self, bus: &mut AddressBus, opcode: u8) {
        match opcode {
            ...
            // SLA
            0x20 => self.shift_left(bus, B),
            0x21 => self.shift_left(bus, C),
            0x22 => self.shift_left(bus, D),
            0x23 => self.shift_left(bus, E),
            0x24 => self.shift_left(bus, H),
            0x25 => self.shift_left(bus, L),
            0x26 => self.shift_left(bus, Direct(HL)),
            0x27 => self.shift_left(bus, A),
            // SRA
